function [ nodeColors ] = plotDNarc_dir( contactSequence, nNodes, ...
    timeInterval)
...
for edges = 1:nEdges
    ...
    if contactSequence(edges,1) < contactSequence(edges,2)
        % then the arrow should be pointing up
        plot(t + 0.7*(cos(theta(51).^2)),...
            min(contactSequence(edges,1:2)) + b*ys(51),'k^',...
            'MarkerFaceColor','k','MarkerSize',4)
    else
        % node 1 > node two and arrow should be pointing down
        plot(t + 0.7*(cos(theta(51).^2)),...
            min(contactSequence(edges,1:2)) + b*ys(51),'kv',...
            'MarkerFaceColor','k','MarkerSize',4)
    end
